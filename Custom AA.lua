-- <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
-- <><> CREATED BY CRIM,SPOEZY,STEPH,JAKE (Main Coder),SODUOSYN AND CONTRIBUTORS ON THE CRIMWALK DISCORD
-- <><> IF YOU HAVE ANY ISSUES OR QUESTIONS WITH MY MESSY CODE, FEEL FREE TO ADD ME ON DISCORD: no, no add.  <><>
-- <><> IF YOU HAVE ANY RECOMMENDATIONS FOR THE CODE, PLEASE DO LEAVE SOME                                   <><>
-- <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
 
-- TABLE OF CONTENTS:
 
-- CONTROL + F TO FIND THEM
-- [LOCALS]                                 A LIST OF ALL THE LOCAL REFERENCES, CLIENTS, ENTITIES, ETC.
-- [ANTIAIM REFERENCES]                     ALL THE REFERENCES OF THE ANTIAIM TAB
-- [LIST OF PRESET ANTIAIMS]                ADD TO THIS LIST IF YOU WOULD LIKE TO MAKE MORE ANTI-AIMS
-- [PRESET ANTIAIMS]                        THIS IS WHERE YOU CAN CHANGE THE PRESETS
-- [FORCE RESET]                            REFERENCES TO RESET ANTIAIMS
-- [RESET BUTTONS]                          LOCALS CREATING NEW BUTTONS
-- [ADAPTIVE ANTIAIMS WHILES]               FUNCTIONS FOR TRYING TO FIND CURRENT FLAGS (THANK YOU RAZER FOR HAVING SOME CODE TO LOOK AT)
-- [DRAW ON SCREEN]                         DRAWS ANTIAIMS/FLAGS ON THE SCREEN
-- [CALLBACKS]                              CALLBACKS
 
-- TO-DO LIST FOR PUBLIC BUILD
    -- MAKE SURE THAT NOTHINGS OVERLY BUGGY
    -- MAKE A CUSTOM ANTIAIM IN GAME/IN MENU THAT PLAYERS CAN SAVE
...
antiaims.edgeyaw_offset = ui.reference("AA", "Anti-aimbot angles", "Edge fake yaw offset")
...
antiaims.twist = ui.reference("AA", "Anti-aimbot angles", "Twist")
...
--local standedgeyaw_offset = interface.slider("Lua", "A", "Stand: Edge fake yaw offset", -180, 180, 0, true)
local standtwist = interface.checkbox("Lua", "A", "Stand: Twist")
...
--local moveedgeyaw_offset = interface.slider("Lua", "A", "Move: Edge fake yaw offset", -180, 180, 0, true)
local movetwist = interface.checkbox("Lua", "A", "Move: Twist")
...
--local jumpedgeyaw_offset = interface.slider("Lua", "A", "Jump: Edge fake yaw offset", -180, 180, 0, true)
local jumptwist = interface.checkbox("Lua", "A", "Jump: Twist")
...
--local slowmotionedgeyaw_offset = interface.slider("Lua", "A", "Slow motion: Edge fake yaw offset", -180, 180, 0, true)
local slowmotiontwist = interface.checkbox("Lua", "A", "Slow motion: Twist")
...
--local crouchedgeyaw_offset = interface.slider("Lua", "A", "Crouch: Edge fake yaw offset", -180, 180, 0, true)
local crouchtwist = interface.checkbox("Lua", "A", "Crouch: Twist")
...
function Standing()
    ...
    --ui_set(antiaims.edgeyaw_offset, ui_get(standedgeyaw_offset))
    --ui_set(antiaims.freestanding, "-")
    ui_set(antiaims.twist, ui_get(standtwist))
...
function Moving()
    ...
    --ui_set(antiaims.edgeyaw, "Off")
    --ui_set(antiaims.freestanding, "-")
    ui_set(antiaims.twist, ui_get(movetwist))
...
function Jumping()
    ...
    --ui_set(antiaims.edgeyaw, "Off")
    --ui_set(antiaims.freestanding, "-")
    ui_set(antiaims.twist, ui_get(jumptwist))
...
function Slowmotion()
    ...
    --ui_set(antiaims.edgeyaw, "Off")
    --ui_set(antiaims.freestanding, "-")
    ui_set(antiaims.twist, ui_get(slowmotiontwist))
...
function Crouching()
    ...
    --ui_set(antiaims.edgeyaw, "Off")
    --ui_set(antiaims.freestanding, "-")
    ui_set(antiaims.twist, ui_get(crouchtwist))
   
end
 
-- ADAPTIVE ANTIAIMS WHILES <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
--local color_picker = ui_new_color_picker("AA", "Other", "Show Flags", 0, 200, 0, 255)
...
local function visible()
    if ui_get(selector) == "Standing" then
        ...
        --ui_set_visible(standedgeyaw_offset, true)
        ui_set_visible(standtwist, true)
    else
        ...
        --ui_set_visible(standedgeyaw_offset, false)
        ui_set_visible(standtwist, false)
    end
    ...
    if ui_get(selector) == "Moving" then
        ...
        --ui_set_visible(moveedgeyaw_offset, true)
        ui_set_visible(movetwist, true)
    else
        ...
        --ui_set_visible(moveedgeyaw_offset, false)
        ui_set_visible(movetwist, false)
    end
    ...
    if ui_get(selector) == "Jumping" then
        ...
        --ui_set_visible(jumpedgeyaw_offset, true)
        ui_set_visible(jumptwist, true)
    else
        ...
        --ui_set_visible(jumpedgeyaw_offset, false)
        ui_set_visible(jumptwist, false)
    end
    ...
    if ui_get(selector) == "Slow motion" then
        ...
        --ui_set_visible(slowmotionedgeyaw_offset, true)
        ui_set_visible(slowmotiontwist, true)
    else
        ...
        --ui_set_visible(slowmotionedgeyaw_offset, false)
        ui_set_visible(slowmotiontwist, false)
    end
    ...
    if ui_get(selector) == "Crouching" then
        ...
        --ui_set_visible(crouchedgeyaw_offset, true)
        ui_set_visible(crouchtwist, true)
    else
        ...
        --ui_set_visible(crouchedgeyaw_offset, false)
        ui_set_visible(crouchtwist, false)
    end 
end

client.set_event_callback('paint', visible)
...
client.set_event_callback("paint", get_antiaim)
